const keiseiStations = {
  "ks-narita-skyaccess": [
    {
      name: "京成高砂",
      id: "keisei-takasago",
    },
    {
      name: "東松戸",
      id: "higashi-matsudo",
    },
    {
      name: "新鎌ヶ谷",
      id: "shinkamagaya",
    },
    {
      name: "千葉ニュータウン中央",
      id: "chiba-newtown-chuo",
    },
    {
      name: "印旛日本医大",
      id: "inba-nihon-idai",
    },
    {
      name: "成田湯川",
      id: "narita-yukawa",
    },
    {
      name: "空港第2ビル（成田第2・第3ターミナル）",
      id: "narita-airport-terminal-2-3",
    },
    {
      name: "成田空港（成田第1ターミナル）",
      id: "narita-airport-terminal-1",
    },
  ],
  "ks-shin-keisei": [
    {
      name: "京成津田沼",
      id: "keisei-tsudanuma",
    },
    {
      name: "新津田沼",
      id: "shin-tsudanuma",
    },
    {
      name: "前原",
      id: "maebara",
    },
    {
      name: "薬園台",
      id: "yakuendai",
    },
    {
      name: "習志野",
      id: "narashino",
    },
    {
      name: "北習志野",
      id: "ks-kita-narashino",
    },
    {
      name: "高根木戸",
      id: "takane-kido",
    },
    {
      name: "高根公団",
      id: "takane-kodan",
    },
    {
      name: "滝不動",
      id: "takifudo",
    },
    {
      name: "三咲",
      id: "misaki",
    },
    {
      name: "二和向台",
      id: "futawa-mukodai",
    },
    {
      name: "鎌ヶ谷大仏",
      id: "kamagaya-daibutsu",
    },
    {
      name: "初富",
      id: "hatsutomi",
    },
    {
      name: "新鎌ヶ谷",
      id: "shinkamagaya",
    },
    {
      name: "北初富",
      id: "kitahatsutomi",
    },
    {
      name: "くぬぎ山",
      id: "kunugiyama",
    },
    {
      name: "元山",
      id: "motoyama",
    },
    {
      name: "五香",
      id: "goko",
    },
    {
      name: "常盤平",
      id: "tokiwadaira",
    },
    {
      name: "八柱",
      id: "yahashira",
    },
    {
      name: "みのり台",
      id: "minoridai",
    },
    {
      name: "松戸新田",
      id: "matsudo-shinden",
    },
    {
      name: "上本郷",
      id: "kamihongo",
    },
    {
      name: "松戸",
      id: "matsudo",
    },
  ],
  "ks-honsen": [
    {
      name: "京成上野",
      id: "keisei-ueno",
    },
    {
      name: "日暮里",
      id: "nippori",
    },
    {
      name: "新三河島",
      id: "shin-mikawashima",
    },
    {
      name: "町屋",
      id: "machiya",
    },
    {
      name: "千住大橋",
      id: "senju-ohashi",
    },
    {
      name: "京成関屋",
      id: "keisei-sekiya",
    },
    {
      name: "堀切菖蒲園",
      id: "horikiri-shobuen",
    },
    {
      name: "お花茶屋",
      id: "ohanajaya",
    },
    {
      name: "青砥",
      id: "aoto",
    },
    {
      name: "京成高砂",
      id: "keisei-takasago",
    },
    {
      name: "京成小岩",
      id: "keisei-koiwa",
    },
    {
      name: "江戸川",
      id: "edogawa",
    },
    {
      name: "国府台",
      id: "kokufu-dai",
    },
    {
      name: "市川真間",
      id: "ichikawa-mama",
    },
    {
      name: "菅野",
      id: "sugano",
    },
    {
      name: "京成八幡",
      id: "keisei-yawata",
    },
    {
      name: "鬼越",
      id: "onigoe",
    },
    {
      name: "京成中山",
      id: "keisei-nakayama",
    },
    {
      name: "東中山",
      id: "higashi-nakayama",
    },
    {
      name: "京成西船",
      id: "keisei-nishifuna",
    },
    {
      name: "海神",
      id: "kaijin",
    },
    {
      name: "京成船橋",
      id: "keisei-funabashi",
    },
    {
      name: "大神宮下",
      id: "daijingu-shita",
    },
    {
      name: "船橋競馬場",
      id: "funabashi-keibajo",
    },
    {
      name: "谷津",
      id: "yatsu",
    },
    {
      name: "京成津田沼",
      id: "keisei-tsudanuma",
    },
    {
      name: "京成大久保",
      id: "keisei-okubo",
    },
    {
      name: "実籾",
      id: "mimomi",
    },
    {
      name: "八千代台",
      id: "yachiyodai",
    },
    {
      name: "京成大和田",
      id: "keisei-owada",
    },
    {
      name: "勝田台",
      id: "katsutadai",
    },
    {
      name: "志津",
      id: "shizu",
    },
    {
      name: "ユーカリが丘",
      id: "ks-yukari-ga-oka",
    },
    {
      name: "京成臼井",
      id: "keisei-usui",
    },
    {
      name: "京成佐倉",
      id: "keisei-sakura",
    },
    {
      name: "大佐倉",
      id: "osakura",
    },
    {
      name: "京成酒々井",
      id: "keisei-shisui",
    },
    {
      name: "宗吾参道",
      id: "sogo-sando",
    },
    {
      name: "公津の杜",
      id: "kozu-no-mori",
    },
    {
      name: "京成成田",
      id: "keisei-narita",
    },
    {
      name: "空港第2ビル（成田第2・第3ターミナル）",
      id: "narita-airport-terminal-2-3",
    },
    {
      name: "成田空港",
      id: "narita-airport-terminal-1",
    },
  ],
  "ks-oshiage": [
    {
      name: "押上（スカイツリー前）",
      id: "oshiage",
    },
    {
      name: "京成曳舟",
      id: "keisei-hikifune",
    },
    {
      name: "八広",
      id: "yahiro",
    },
    {
      name: "四ツ木",
      id: "yotsugi",
    },
    {
      name: "京成立石",
      id: "keisei-tateishi",
    },
    {
      name: "青砥",
      id: "aoto",
    },
  ],
  "ks-kanamachi": [
    {
      name: "京成高砂",
      id: "keisei-takasago",
    },
    {
      name: "柴又",
      id: "shibamata",
    },
    {
      name: "京成金町",
      id: "keisei-kanamachi",
    },
  ],
  "ks-chiba": [
    {
      name: "京成津田沼",
      id: "keisei-tsudanuma",
    },
    {
      name: "京成幕張本郷",
      id: "keisei-makuharihongō",
    },
    {
      name: "京成幕張",
      id: "keisei-makuhari",
    },
    {
      name: "検見川",
      id: "kemigawa",
    },
    {
      name: "京成稲毛",
      id: "keisei-inage",
    },
    {
      name: "みどり台",
      id: "midoridai",
    },
    {
      name: "西登戸",
      id: "nishi-nobuto",
    },
    {
      name: "深千葉",
      id: "shin-chiba",
    },
    {
      name: "京成千葉",
      id: "keisei-chiba",
    },
    {
      name: "千葉中央",
      id: "chiba-chuo",
    },
  ],
  "ks-chihara": [
    {
      name: "千葉中央",
      id: "chiba-chuo",
    },
    {
      name: "千葉寺",
      id: "chiba-dera",
    },
    {
      name: "大森台",
      id: "oomoridai",
    },
    {
      name: "学園前",
      id: "gakuenmae",
    },
    {
      name: "おゆみ野",
      id: "oyumino",
    },
    {
      name: "ちはら台",
      id: "chiharadai",
    },
  ],
  "ks-higashi-narita": [
    {
      name: "京成成田",
      id: "keisei-narita",
    },
    {
      name: "東成田",
      id: "higashi-narita",
    },
  ],
  "ks-shibayama": [
    {
      name: "東成田",
      id: "higashi-narita",
    },
    {
      name: "芝山千代田",
      id: "shibayama-chiyoda",
    },
  ],
};
Object.keys(keiseiStations).forEach((key) => {
  const stations = keiseiStations[key];
  const stationElement = document.getElementById(key);
  let stationElements = "";
  stationElements += '<div class="ml-4 text-lg">';
  stations.forEach((station) => {
    stationElements += stationTemplate(station, key);
  });
  stationElements += "</div>";
  stationElement.innerHTML += stationElements;
  const stationToggle = document.getElementById(`${key}-toggle`);
  stationToggle.addEventListener("click", (event) => {
    const checkboxes = document.querySelectorAll(
      `#${key} input[type="checkbox"]`
    );
    checkboxes.forEach((checkbox) => {
      checkbox.checked = event.target.checked;
    });
  });
});
const ksToggle = document.getElementById("ks-toggle");
ksToggle.addEventListener("click", () => {
  const checkboxes = document.querySelectorAll("#ks input[type='checkbox']");
  checkboxes.forEach((checkbox) => {
    checkbox.checked = ksToggle.checked;
  });
});
